...
import doctorImage2 from "@/assets/img/dr-saurodip.jpg";
...
const DoctorCardSecond = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);

  const doctorDetails = {
    name: "Dr. Saurodip Maity",
    image: doctorImage2,
    specialization: "Paediatric Intensive Care, Critical Care, Emergency Paediatrics",
    ...
    contact: (
      <>
        <p>
          Dr Saurodip Maity is a Consultant Paediatrician and Paediatric
          Intensivist with over eight years of experience in managing infants,
          children, and adolescents, including complex and critical conditions.
        </p>

        <p>
          Trained in India and the UK, he holds an MBBS, DNB in Paediatrics,
          IDPCCM certification, and specialised fellowship training in paediatric
          intensive care.
        </p>

        <p>
          Dr Saurodip follows an evidence-based and precision-driven approach,
          combining advanced clinical judgement with compassionate communication
          to support both children and families.
        </p>

        <p>
          He is committed to delivering high-quality critical care, early
          intervention, and structured paediatric management to improve outcomes
          and ensure long-term child health.
        </p>
      </>
    ),
  };
  ...
  const expertiseTags = [
    ...
    "ECMO",
    "Transplants",
    "Emergency paediatric resuscitation",
  ];
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "contact") updatedValue = value.replace(/\D/g, "").slice(0, 10);
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/book-appointment.php`, {
        ...
        body: JSON.stringify({
          ...
          message: "",
          landing: "paediatric-care-landing",
      ...
      if (data.success) {
        const thankYouData = {
          ...
          doctorId: DOCTOR_ID,
          lastPage: "/paediatric-care-evolution"
        };
        ...
        navigate("/paediatric-care-evolution/thank-you");
      } else {
        setErrors((prev) => ({
          ...prev,
  ...
  return (
    <section id="doctor-card-2" className="bg-gradient-to-b from-gray-50 to-white pt-8 pb-8">
      <style>{`
        .date-scrollbar::-webkit-scrollbar { height: 4px; }
        .date-scrollbar::-webkit-scrollbar-track { background: transparent; }
                    ...
                    >
                      ...
                      <div className="mb-3">
                        <div className="inline-flex items-center gap-2 rounded-full border border-[#00B4CC]/25 px-4 py-2 bg-[#00B4CC]/[0.04]">
                          ...
                          <span className="text-sm font-medium text-gray-700">Whitefield Center</span>
                        </div>
                      </div>
                      ...
                      <div className="mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          <button type="button" onClick={() => scrollDates("left")} className="h-8 w-8 shrink-0 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:border-[#00B4CC] hover:text-[#00B4CC] transition">
                            <ChevronLeft className="w-4 h-4" />
                          </button>

                          <div ref={dateScrollRef} className="date-scrollbar flex-1 overflow-x-auto">
                            <div className="flex gap-1 min-w-max pr-1 pb-1">
                              {loadingDates ? (
                                <div className="px-3 py-2 text-sm text-gray-500">Loading dates...</div>
                              ) : dateOptions.length > 0 ? (
                                dateOptions.map((item) => {
                                  const active = selectedDate?.dateValue === item.dateValue;
                                  return (
                                    <button
                                      key={item.dateValue}
                                      type="button"
                                    ...
                                    >
                                      <div className="text-[11px] font-medium">{item.day}</div>
                                      <div className="text-lg font-bold leading-tight mt-1">{item.date}</div>
                                      <div className={`text-[9px] leading-tight mt-1 ${active ? "text-white/90" : "text-gray-500"}`}>
                                        {item.monthYear || ""}
                                      </div>
                                    </button>
                                  );
                                ...
                                <div className="px-3 py-2 text-sm text-gray-500">No dates available</div>
                              )}
                            </div>
                          </div>

                          <button type="button" onClick={() => scrollDates("right")} className="h-8 w-8 shrink-0 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:border-[#00B4CC] hover:text-[#00B4CC] transition">
                      ...
                      <div className="mb-5">
                        ...
                        <div className="flex items-center gap-2">
                          <button type="button" onClick={() => scrollSlots("left")} className="h-8 w-8 shrink-0 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:border-[#00B4CC] hover:text-[#00B4CC] transition">
                            <ChevronLeft className="w-4 h-4" />
                          </button>

                          <div ref={slotScrollRef} className="date-scrollbar flex-1 overflow-x-auto">
                            <div className="flex gap-2 min-w-max pr-1 pb-1">
                              {loadingSlots ? (
                                <div className="px-3 py-2 text-sm text-gray-500">Loading slots...</div>
                                ...
                                <div className="px-3 py-2 text-sm text-gray-500">No slots available</div>
                              )}
                            </div>
                          </div>

                          <button type="button" onClick={() => scrollSlots("right")} className="h-8 w-8 shrink-0 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:border-[#00B4CC] hover:text-[#00B4CC] transition">
                            <ChevronRight className="w-4 h-4" />
                          </button>
                        </div>
                      ...
                      <div className="rounded-2xl bg-green-50 border border-green-200 px-4 py-3 mb-5">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-gray-900">Next booking slot</p>
                            <p className="text-sm text-gray-700">
                              {selectedDate?.full || "No date selected"} • {selectedSlot?.label || "No slot selected"}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <Button
                        onClick={() => {
                          ...
                          if (Object.keys(newErrors).length === 0) setBookingStep("form");
                        }}
                  ...
                  {bookingStep === "form" && (
                    ...
                    >
                      ...
                      <div className="rounded-2xl bg-[#00B4CC]/[0.04] border border-[#00B4CC]/20 px-4 py-3 mb-3">
                        <p className="text-sm font-semibold text-gray-900">Selected Appointment</p>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full rounded-xl border px-4 py-3 text-sm outline-none transition ${
                                errors.name ? "border-red-400 focus:border-red-500" : "border-gray-200 focus:border-[#00B4CC]"
                              }`}
                            />
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full rounded-xl border px-4 py-3 text-sm outline-none transition ${
                                errors.contact ? "border-red-400 focus:border-red-500" : "border-gray-200 focus:border-[#00B4CC]"
                              }`}
                            />
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full rounded-xl border px-4 py-3 text-sm outline-none transition ${
                                errors.email ? "border-red-400 focus:border-red-500" : "border-gray-200 focus:border-[#00B4CC]"
                              }`}
                            />
...
export default DoctorCardSecond;